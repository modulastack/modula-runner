...
class RuntimeHandle implements RunnerRuntimeHandle {
  ...
  private async shutdown(): Promise<RunnerStopResult> {
    ...
    const result = await this.cleanup()
    // Cleanup that could not confirm a child leaves that child's dispatch parked for good, so the
    // finish records are traded for terminating on the first signal; the result already says so.
    if (result.status === 'confirmed') await this.drain()
    this.finish(result)
    return result
  }

  // A launch parked on its session's end, or a recovery that re-adopted one, drains only once
  // cleanup kills that session, so shutdown gives work one turn rather than waiting for all of it.
  // Both waits span the dispatch set, not just the lifecycle queue that dispatch and recovery
  // left: a queue-only wait returns at once and silently drops the finish records the
  // post-cleanup wait exists to land.
  private drainReady(): Promise<unknown> {
    return Promise.race([this.drain(), new Promise(resolve => setImmediate(resolve))])
  }
