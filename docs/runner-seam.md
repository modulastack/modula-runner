...
| Preview servers and browser-QA targets | **On a namespace-capable Linux host, off-loopback exposure is prevented, not merely detected**: the preview runs in a loopback-only network namespace, so even a `0.0.0.0` bind is unreachable from every non-loopback address for the preview's whole lifetime. Where namespaces are denied, or on macOS, the runner degrades visibly to detecting and terminating an off-loopback listener and says so — a fallback, never a silent claim of prevention. Reachability for the operator is a **local loopback forward, never a cross-machine tunnel**: an unprivileged runner cannot re-enter the preview's namespace, so it hands a host-loopback listening socket into that namespace and reports the resulting host-loopback forward port; the operator's browser — same machine (split) or same host (co-resident) — reaches it over loopback, and nothing off-machine can. The binding is still verified against the process's real listening sockets before a port is reported, because a command is free to bind more than it was configured with. Only the port crosses — a host or URL would be an endpoint. (v2) A co-resident affordance by design: a remote client reached through the relay cannot resolve the runner's loopback, so remote sessions surface previews as local-to-the-runner-host with an explicit reason, never a dead link; a sealed preview tunnel over the runner's outbound connection is future work, and no preview traffic transits Modula. |
| Preview process ownership | Previews are spawned into their own process group, and the runner tracks them by group membership as well as parentage — a wrapper that spawns a detached child and exits leaves that child adopted by init, where no ancestry walk finds it again. A descendant that calls `setsid` escapes both tracking mechanisms; its **network exposure is nonetheless contained**, because the loopback-only namespace it inherits holds across reparenting and `setsid` — OS network isolation is the only thing that does, which is why it is the mechanism. What is still not guaranteed is **termination** of such an escaped descendant: closing that needs a process containment unit (a cgroup on Linux, no clean equivalent on macOS) and remains deferred. The escape requires a recipe the operator installed locally, since the runner's own recipes are the only commands it starts. |
| Preview exposure before verification | On a namespace-capable Linux host the preview is spawned **inside** the loopback-only namespace, so there is no host-namespace window in which an off-loopback bind is reachable — the isolation is in place before the process runs. Where namespaces are denied, or on macOS, the preview runs in the host namespace and the pre-verification window returns: readiness polls from 50 ms, the tree is settled before readiness is granted, and the sweep re-checks continuously, which bounds but does not eliminate it. Stated rather than implied: on those hosts the exposure requires a recipe the operator installed locally, since the control plane cannot supply a command line. |
| Preview working directory | The grant is checked on the resolved real path, the command is spawned into that resolved path rather than the caller's, and on Linux the running process's own working directory is read back and re-checked against the same grant the admission used. A platform without that read-back, or a command that exits before it can be read, keeps a narrow window between resolving a path and entering it; the resolved-path spawn is the guarantee for that case, and a reliable pre-command working-directory capture that would close it is tracked (#16). |
| Pane working directory | The same as a preview: the grant is checked on the resolved real path, the pane launches in that resolved path, and on Linux the pane process's own working directory is read back through its pid and re-checked against the admission's grant — a pane found outside its grant is terminated and audited `path-not-granted` before it is reported ready. The read-back and the admission share one consent object by construction, so they cannot judge a moved landing against different grant sets. The fast-exit residual above applies equally: a command gone before the read-back leaves no working directory to read, and closing that needs the reliable capture tracked in #16. |
...
| The local floor | Command allowlist (ships signed, editable only locally), per-directory consent, kill switch, append-only local audit log. Enforced here, configurable only here. The allowlist governs the commands the runner is *asked* to run; the runner's own bootstrap machinery for running them safely — the network-namespace containment mechanism (`unshare`/`ip`) and its capability probe — is runner-owned infrastructure derived from the process environment, not a selectable workload on the allowlist, so it is not gated by it. |
