...
| `modula-runner allowlist init/sign/verify` | Creates an operator-held signing key at an explicit path outside runner state, writes only the public anchor and signed policy into runner state, refuses overwrite, and lets later edits be signed without the `run` process ever opening the private key. |
...
local model profiles, configured endpoints, signed allowlist, public trust anchors, and append-only
audit log. The implementation module owns filenames and atomic writes; callers receive one home
interface rather than independent path knobs. `profile` and `endpoint` are the public mutation
...
- **A2.1:** a permitted launch request resolves a local profile and granted target; the runner
  opens exactly one terminal channel and starts the intended command in the deterministic
  worktree. Unknown or ungranted input is refused without spawning.
...
- **A4.4:** each runtime capability contains `runtime`, `version` or `null`, auth state, and supported
  ...
  8 endpoints, 64 model names per endpoint, 128 characters per model, and 64 per version; visible
  truncation announces the true total through `modelCount`. A local change is advertised
  within one configured refresh interval (default 60 seconds, minimum 500 ms), while endpoint
...
- **A5.5:** every workload, probe, git/tmux, preview, and pane spawn routed through the seam,
  plus each refusal, outcome, policy rejection, and kill, is append-only and durable before
  acknowledgement. Containment bootstrap (`unshare`/`ip`) remains the seam contract’s explicit
  runner-infrastructure exception and is audited separately. Concurrent writers do not truncate,
  reorder a single operation, or report green on omission.
...
- **A5.7:** corrupted state, full disk, permission failure, interrupted atomic write, hostile output,
  and control-character input fail closed with bounded sanitized diagnostics.
