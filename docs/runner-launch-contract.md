...
`content-type: application/json`; responses over 64 KiB or with a non-JSON redemption body are
malformed. Parsers copy only declared fields; unknown fields cannot influence state and provide
additive evolution room.
...
At `confirmationExpiresAt`, a still-pending runner performs one final exact confirmation. A `204`
settles it; authoritative `409` or `410` marks it revoked/expired locally. A final route-absent
...
Branch fields are non-empty, at most 255 characters, control-character-free, and must pass local
`git check-ref-format --branch` before acceptance. `relativeCwd` is at most 1,024 characters, uses
...
One accepted request opens at most one terminal channel. The runner may emit `open` before
`SESSION_STARTED`; the counterpart buffers that channel without sending `INIT` until the correlated
started result arrives. If the request does not reach `SESSION_STARTED`, the runner closes that
exact channel before reporting failure and the counterpart discards the buffered channel on close.
`SESSION_STARTED` is sent only after the child/session and receipt are
sufficiently durable to replay honestly. Terminal `READY` then carries the existing session
metadata, and `INIT.profile`, if present, must equal `terminalProfile`; it cannot reselect
`modelProfileId`.
...
resume until the maximum deadline of every request refused under the block has passed.

The append-only audit file has a fixed 64 KiB aggregate budget and never reflects raw refusal input.
No audit record is automatically evicted. If the next required refusal record cannot fit, the runner
admits/spawns nothing, emits at most the bounded connection-level `storage-unavailable` error, and
closes job control. Exhaustion therefore becomes visible fail-close rather than disk growth or an
undurable correlated refusal.
...
- A `spawn-intent` with the exact matching owned live tmux session may be adopted under the same
  stable session id and a newly minted terminal channel; the replayed `SESSION_STARTED` carries the
  new channel id.
...
- Deadline expiry triggers bounded cleanup. It emits `launch-timeout` only when cleanup proves no
  launch-owned child/session remains. Unconfirmed cleanup durably enters `uncertain`, emits
  `recovery-uncertain`, preserves the operator inspection/kill path, and forbids automatic redrive
  or a new id for the same work.
